...
// Singleton pattern ile tek bir istemci oluştur
let supabaseInstance: SupabaseClient | null = null;

export const getSupabase = (): SupabaseClient => {
  if (!supabaseInstance) {
    if (!supabaseUrl || !supabaseKey) {
      console.error('Supabase bağlantısı yapılamadı: URL veya API Anahtarı eksik!');
      throw new Error('Supabase URL ve Key tanımlanmalıdır');
    }
    
    // Bağlantı değerlerini konsola yazdır (sadece geliştirme sırasında)
    ...
    try {
      supabaseInstance = createClient(supabaseUrl, supabaseKey, {
        ...
        global: {
          headers: {
            'X-Client-Info': 'AR Learning Platform',
          },
        },
      });
